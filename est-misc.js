...
function collectLineItems() {
  ...
  // 되니?????????"): 위 "재계산으로 다시 만들어지므로 제외"가, 사용자가
  // 그 자동계산 단가를 직접 고쳐놓은(manualOverride) 경우까지 무조건
  // 저장에서 빼버려서 실제로 수정한 값 자체가 유실되고 있었음 - 자동
  // 생성 행이라도 manualOverride가 있으면 "어떤 자동유형이었는지"
  // (autoType)까지 함께 저장해서, 복원 시 그 자동계산 로직이 이 값을
  // 다시 찾아서 유지할 수 있게 함.
  document.querySelectorAll('#svc-body tr').forEach(function(tr){
    var isAutoGenerated = tr.hasAttribute('data-rail-src') || tr.hasAttribute('data-railcost-src') || tr.hasAttribute('data-svc-type');
    if (isAutoGenerated && !tr.dataset.manualOverride) return;
    var content = tr.querySelector('.svc-content')?.value || '';
    var price = getPriceVal(tr.querySelector('.sprice'));
    if (!content && !price) return;
    var autoType = '';
    if (isAutoGenerated && tr.dataset.manualOverride) {
      if (tr.hasAttribute('data-rail-src')) autoType = 'rail';
      else if (tr.hasAttribute('data-railcost-src')) autoType = 'railcost';
      else {
        var svcType = tr.getAttribute('data-svc-type');
        if (svcType === '실측비') autoType = 'measure';
        else if (svcType === '시공비') autoType = 'install';
        else if (svcType === '블라인드시공') autoType = 'blindInstall';
        else if (svcType === '옵션추가금') autoType = 'option';
      }
    }
    lineItems.push({
...
function collectFormData() {
  ...
  // editingEstDbId만 사라져서, 다음 저장이 "수정"이 아니라 "새로 생성"으로
  // 처리되어 같은 견적이 계속 중복 생성됐음. 초안에 같이 저장해서 복원 가능하게 함.
  // 2026-09-15: 초안 JSON 키 이름도 window._estEditState 프로퍼티명(밑줄 없는
  // 캐멀케이스)과 통일 - 예전엔 여기만 밑줄 붙은 이름을 따로 썼음.
  form.editingEstDbId = window._estEditState.editingEstDbId || null;
  form.editingEstUpdatedAt = window._estEditState.editingEstUpdatedAt || null;
  if (form.custType === 'as') {
...
function loadDraft() {
  try {
    ...
    if (confirm('저장된 임시 초안이 있습니다.\n불러오시겠습니까?\n(' + savedAt.toLocaleString('ko-KR') + ')')) {
      ...
      if (d.editingEstDbId) {
        window._estEditState.editingEstDbId = d.editingEstDbId;
        window._estEditState.editingEstUpdatedAt = d.editingEstUpdatedAt || null;
      }
      ...
      if (Array.isArray(d.lineItems) && d.lineItems.length > 0) {
        ...
        d.lineItems.forEach(function(item) {
          if (item.type === 'curtain') {
            ...
            // 2026-09-19(선혜님 - "다시 열어보니 실측+레일비가... 이게
            // 말이 되니?????????"): est-customer-load.js의 정식 복원과
            // 짝을 맞춰서, 임시저장 복원에도 자동계산 항목 수동수정
            // (autoType) 반영 - 안 그러면 저장은 안 됐어도 입력 도중
            // 새로고침만 해도 방금 수정한 값이 자동계산값으로 되돌아감.
            if (item.autoType) {
              str.dataset.manualOverride = '1';
              if (item.autoType === 'rail' || item.autoType === 'railcost') {
                var matchedCurtainTrDraft = Array.from(document.querySelectorAll('#curtain-body tr')).find(function(ctr) {
                  return (ctr.querySelector('.space-inp')?.value || '') === (item.space || '') && item.space;
                });
                if (matchedCurtainTrDraft) {
                  if (!matchedCurtainTrDraft.dataset.rowUid) {
                    window._curtainRowSeq = (window._curtainRowSeq || 0) + 1;
                    matchedCurtainTrDraft.dataset.rowUid = 'crow' + window._curtainRowSeq;
                  }
                  str.setAttribute(item.autoType === 'rail' ? 'data-rail-src' : 'data-railcost-src', matchedCurtainTrDraft.dataset.rowUid);
                }
              } else if (item.autoType === 'measure') {
                str.setAttribute('data-svc-type', '실측비');
              } else if (item.autoType === 'install') {
                str.setAttribute('data-svc-type', '시공비');
                str.setAttribute('data-install-base', String(item.price || 0));
              } else if (item.autoType === 'blindInstall') {
                str.setAttribute('data-svc-type', '블라인드시공');
              } else if (item.autoType === 'option') {
