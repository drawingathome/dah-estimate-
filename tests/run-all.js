...
if (/dah-dashboard/.test(target)) {
  ...
  // 2026-09-21: "미수금" 계산이 견적서(dah_saved)와 고객레벨(customers
  ...
  // 2026-09-21(근본원인 수정 - 김은/황남주 실사례): 결제탭이 "이 고객
  // 견적서가 있는지"를 로컬 캐시로만 판단하다가, 서버엔 있는데 로컬
  // 캐시엔 없는 경우(다른 기기 등) 예전 고객레벨 방식으로 조용히
  // 되돌아가던 문제 - 서버로 재확인 후 자동 교정하는지 검증
  scripts.push(['pay_tab_server_verify_check.js', []]);
  // 2026-09-21(선혜님 - "안전하게 해야지" 지시로 신설): 서버 재확인
  // 응답이 도착하는 도중 사용자가 이미 입력을 시작했으면 화면을
  // 재렌더링하지 않고 입력값을 보호하는지 검증.
  scripts.push(['pay-tab-input-protection-check.js', []]);
  ...
  // 2026-09-22(선혜님 - "더하기 계산이 안맞는게 더 문제야" - 최금희 고객
  // 견적서 실사례): 요약카드(renderSvcSummary)가 행 추가/자동생성 때만
  // 갱신되고, 기존 행 단가를 직접 고치거나 복제/삭제할 때는 안 갱신되던
  // 문제 + 복제본이 자동관리 태그를 물려받아 원본과 혼선 생기던 문제
  scripts.push(['svc-summary-sync-check.js', []]);
  // 2026-09-22(진짜 근본원인 발견 - "서울 시공비 40,000원" 사건): 실측비
  // 칸 옆 시공비 칸에 실수로 다른 값을 잘못 입력해도 화면상 자동계산값과
  // 구분이 안 되던 문제 - 직접수정시 눈에 띄는 표시가 붙는지 검증
  scripts.push(['manual-override-visual-marker-check.js', []]);
  // 2026-09-22(선혜님 - "동호수가 한번에 다 보임... 예전에도 말한거잖아"):
  // 9/16 도입된 상세주소 자동분리가 "OOO동 OOO호" 형식만 다뤄서, "트리니원
  // 112-1804"처럼 단지명+숫자-숫자 형식은 원래부터 미지원이었음 - 지번주소
  // 오분리 없이 안전하게 확장
  scripts.push(['split-addr-detail-check.js', []]);
  ...
  // 2026-09-22(선혜님 - "2번과 3번 확실히 고쳐" - 주소분리가 9/15 두 번,
  // 오늘까지 세 번 재발한 근본 대응): 추측 알고리즘을 더 다듬는 대신,
  // 주소검색 완료 즉시 상세주소 칸으로 자동 포커스 이동시켜 애초에
  // 추측이 필요 없게 함
  scripts.push(['addr-detail-autofocus-check.js', []]);
  // 2026-09-22(선혜님 - "위에 중복된 오류들 정리한것 해결을 먼저" - 체크리스트
  // 35번 "발주서 카테고리는 항상 세트"가 자동검사 없이 글로만 있던 것 확인
  // 후 신설): 원단/캔가공소/레일자재/블라인드 4개 카테고리가 항상 일관되게
  // 처리되는지(전체보기에서 특정 카테고리만 누락되는 회귀 등) 검증
  scripts.push(['vendor-category-set-consistency-check.js', []]);
  // 2026-09-22(선혜님 - "위는 확실히 직접 확인하고 근본적으로 처리한거니"
  // 재확인 - M번(iOS 지연호출)이 grep 정적분석만이었던 걸 인정하고 실제
