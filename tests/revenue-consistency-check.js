...
async function run() {
  ...
  async function testOnDevice(width, label) {
    ...
    // 2026-09-01(선혜님 지시 - "표현이 명확하지 않은데"): "현재 매출"이
    // "N월 매출(진행중)"으로 라벨이 바뀜(실제 월 숫자를 넣어 명확하게) -
    // 월 숫자는 테스트 실행 시점에 따라 달라지니 숫자 부분은 와일드카드로.
    const homeMatch = homeText.match(/\d+월 매출\(진행중\)([\d,]+)원/);
