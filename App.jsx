...
export default function App() {
  ...
  const [form, setForm] = useState(()=>{
    ...
    return {
      ...
      clientName:survey.name||"", clientPhone:survey.phone||"", clientAddr:survey.addr||"",
      ...
      repairType:"", repairNote:"", memo:"", balOverride:"",
      surveyData: survey,
    };
  });
  ...
  const resetAll = () => {
    ...
    setForm({
      ...
      repairType:"",repairNote:"",memo:"",balOverride:"",surveyData:null
    });
  ...
  return (
    <div style={{minHeight:"100vh",background:"#F7F4F0",fontFamily:FONT,paddingBottom:80,maxWidth:794,margin:"0 auto"}}>
      ...
      {/* 고정 헤더 */}
      <div style={{position:"sticky",top:0,zIndex:100,background:"#fff",borderBottom:`1px solid ${C.border}`,padding:"10px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",maxWidth:794,margin:"0 auto"}}>
        <img src={LOGO} alt="DRAWING at HOME" style={{height:18,opacity:1}}/>
        <div style={{display:"flex",gap:6,alignItems:"center"}}>
          {[["estimate","가견적서"],["final","최종견적서"]].map(([v,l])=>(
            <button key={v} onClick={()=>{
              sf("type",v);
              // 미리보기 단계에 있으면 마지막 단계로 유지, 아니면 1단계로
              if(section!=="미리보기") setStep(1);
            }} style={{padding:"6px 12px",fontSize:11,fontWeight:700,border:`1px solid ${form.type===v?C.orange:C.border}`,borderRadius:4,background:form.type===v?C.orange:"#fff",color:form.type===v?"#fff":C.dark,cursor:"pointer"}}>{l}</button>
          ))}
          <button onClick={()=>{setAdminOpen(true);setAdminAuth(false);setAdminPw("");}} style={{padding:"6px 8px",fontSize:13,border:`1px solid ${C.border}`,borderRadius:4,background:"#fff",cursor:"pointer",color:C.mid}} title="관리자 설정">⚙️</button>
        </div>
      </div>
